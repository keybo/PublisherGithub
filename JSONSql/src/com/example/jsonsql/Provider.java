...
public class Provider extends ContentProvider 
{
	...
	// public static final String PROJECTS_MIME_TYPE =
	// ContentResolver.CURSOR_DIR_BASE_TYPE
	// + "/Project";
	// public static final String PROJECT_MIME_TYPE =
	// ContentResolver.CURSOR_ITEM_BASE_TYPE
	// + "/Project";
	// public static final String

	public static final String AUTHORITY = "com.example.jsonsql.provider";
	...
	static {
		...
		// map.put(Database.C_ARTICLEDESCRIPTION,
		// Database.C_ARTICLEDESCRIPTION);
	...
	public String getType(Uri uri) {
		...
		case PROJECT:
			return "vnd.android.cursor.item/vnd.com.example.provider.project";
		case PROJECTS:
			return "vnd.android.cursor.dir/vnd.com.example.provider.project";
	...
	public Uri insert(Uri uri, ContentValues values) 
	{
		...
		/*long newId = 0;
		int option = matcher.match(uri);
		SQLiteDatabase database = mDB.getWritableDatabase();
		Cursor cursor = null;
		switch (option) {
		case PROJECT:

			values.put(Database.C_ARTCATEGORY, cursor.getString(1));
			values.put(Database.C_ARTICLEDESCRIPTION, cursor.getString(2));
			newId = database.insert(Database.NAME, null, values);
			Log.e("Error from json sql", "Error from json sql" + newId);
			// queryBuilder.setTables(Database.NAME);
	 ...
	 * @SuppressWarnings("null")
	 * 
	 * @Override public Uri insert(Uri arg0, ContentValues values) { long newId;
	 * 
	 * int option = matcher.match(arg0);
	 * 
	 * SQLiteDatabase database = mDB.getWritableDatabase(); switch(option) {
	 * case PROJECTS:
	 * 
	 * case PROJECT: Cursor cursor = null; do {
	 * 
	 ...
	 * 
	 * } while(cursor.moveToFirst()) ; cursor.close();
	 * newId=database.insert(Database.NAME, null, values); break; default: throw
	 * new IllegalArgumentException("Illegal insert"); }
	 * 
	 * if (newId > -1) { getContext().getContentResolver().notifyChange(arg0,
	 * null); return ContentUris.withAppendedId(arg0, newId); } else { throw new
	 * SQLException("Could not insert row into " + arg0); } }
	 */
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		Cursor cursor = builder.query(mDB.getReadableDatabase(), projection,
	...
	public int update(Uri uri, ContentValues cv, String selection,
		...
		return mDB.getWritableDatabase().update(table, cv, selection,
				selectionArgs);
	}
