...
public class EntryActivity extends ProgressActivity {
	...
	private static final TranslateAnimation SLIDE_IN_RIGHT = generateAnimation(
			1, 0);
	private static final TranslateAnimation SLIDE_IN_LEFT = generateAnimation(
			-1, 0);
	private static final TranslateAnimation SLIDE_OUT_RIGHT = generateAnimation(
			0, 1);
	private static final TranslateAnimation SLIDE_OUT_LEFT = generateAnimation(
			0, -1);
	...
	private static final String BACKGROUND_COLOR = PrefUtils.getBoolean(
			PrefUtils.LIGHT_THEME, true) ? "#f6f6f6" : "#181b1f";
	private static final String TEXT_COLOR = PrefUtils.getBoolean(
			PrefUtils.LIGHT_THEME, true) ? "#000000" : "#C0C0C0";
	private static final String BUTTON_COLOR = PrefUtils.getBoolean(
			PrefUtils.LIGHT_THEME, true) ? "#D0D0D0" : "#505050";
	...
	private static final String BODY_START = CSS
			+ "<body link='#97ACE5' text='" + TEXT_COLOR + "'>";
	private static final String FONTSIZE_START = CSS + BODY_START
			+ "<font size='+";
	...
	private static final String BUTTON_END = "' style='background-color:"
	...
	private static final String LINK_BUTTON_MIDDLE = "' style='background-color:"
			+ BUTTON_COLOR
			+ "; color:"
			+ TEXT_COLOR
	...
	private int titlePosition, datePosition, mobilizedHtmlPosition,
			abstractPosition, linkPosition, feedIdPosition, isFavoritePosition,
	...
	private final ThrottledContentObserver mTasksObserver = new ThrottledContentObserver(
			new Handler(), 2000) {
		@Override
		public void onChangeThrottled() {
			boolean isMobilizing = true;// FetcherService.getMobilizingTaskId(_id)
										// != -1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mobilizedHtmlPosition = entryCursor
		...
		isFavoritePosition = entryCursor
				.getColumnIndex(EntryColumns.IS_FAVORITE);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putBoolean(SAVE_INSTANCE_IS_FULLSCREEN, !getActionBar()
				.isShowing());
		...
		outState.putFloat(SAVE_INSTANCE_SCROLL_PERCENTAGE,
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		mEntriesIds = savedInstanceState
				.getLongArray(SAVE_INSTANCE_ENTRIES_IDS);
		mScrollPercentage = savedInstanceState
				.getFloat(SAVE_INSTANCE_SCROLL_PERCENTAGE);
	...
	protected void onResume() {
		...
		try {
			webView.onResume();
		} catch (Exception unused) { // Seems possible to have an NPE here on
										// some phones...
		}
	...
	private void reload(boolean forceUpdate) {
		...
		if (entryCursor.moveToFirst()) {
			...
			Cursor cursor = cr.query(FeedColumns.CONTENT_URI(feedId),
					new String[] { FeedColumns.NAME, FeedColumns.URL }, null,
					null, null);
			if (cursor.moveToFirst()) {
				setTitle(cursor.isNull(0) ? cursor.getString(1) : cursor
						.getString(0));
			...
			if (iconBytes == null || iconBytes.length == 0) {
				Cursor iconCursor = cr.query(
						FeedColumns.CONTENT_URI(Integer.toString(feedId)),
						new String[] { FeedColumns._ID, FeedColumns.ICON },
						null, null, null);
			...
			if (iconBytes != null && iconBytes.length > 0) {
				...
				if (bitmap != null) {
					...
					// getActionBar().setIcon(R.drawable.action_export);
				}
			} else {
				// getActionBar().setIcon(R.drawable.action_export);
			}
			...
			// html-header
			boolean localPictures = contentText
					.contains(Constants.IMAGEID_REPLACEMENT);

			if (localPictures) {
				contentText = contentText.replace(
						Constants.IMAGEID_REPLACEMENT, _id
								+ Constants.IMAGEFILE_IDSEPARATOR);
			}
			...
			if (PrefUtils.getBoolean(PrefUtils.DISABLE_PICTURES, false)) {
				...
				if (webView.getSettings().getBlockNetworkImage()) {
					// setBlockNetwortImage(false) calls postSync, which takes
					// time, so we clean up the html first and change the value
					// afterwards
			...
			// }
			String idfordisqus = String.valueOf(feedId);
			String name = "babu001";
			webView.loadDataWithBaseURL(
					"",
					generateHtmlContent(title, link, contentText, enclosure,
							author, timestamp, idfordisqus, name), TEXT_HTML,
					Constants.UTF8, null); // do
			// not
			// put
			// 'null'
			// to
			...
			/*long mobilizingTaskId = 2;// FetcherService.getMobilizingTaskId(_id);
			if (mobilizingTaskId != -1) {
				getProgressBar().setVisibility(View.VISIBLE);
				cr.unregisterContentObserver(mTasksObserver);
				cr.registerContentObserver(
						TaskColumns.CONTENT_URI(mobilizingTaskId), false,
						mTasksObserver);
			} else {
	...
	private String generateHtmlContent(String title, String link,
			String abstractText, String enclosure, String author,
			long timestamp, String feedid, String disqusname) {
		StringBuilder content = new StringBuilder();

		int fontSize = Integer.parseInt(PrefUtils.getString(
				PrefUtils.FONT_SIZE, "0"));
		if (fontSize > 0) {
			content.append(FONTSIZE_START).append(fontSize)
					.append(FONTSIZE_MIDDLE);
		} else {
		...
		content.append(TITLE_START).append(link).append(TITLE_MIDDLE)
				.append(title).append(TITLE_END).append(SUBTITLE_START);
		...
		content.append(dateStringBuilder).append(SUBTITLE_END)
				.append(abstractText).append(BUTTON_SEPARATION)
				.append(BUTTON_START);

		if (!preferFullText) {
			content.append(getString(R.string.get_full_text))
					.append(BUTTON_MIDDLE)
					.append("injectedJSObject.onClickFullText();");
		} else {
			content.append(getString(R.string.original_text))
					.append(BUTTON_MIDDLE)
					.append("injectedJSObject.onClickOriginalText();");
		}
		content.append(BUTTON_END);

		if (enclosure != null && enclosure.length() > 6
				&& !enclosure.contains(IMAGE_ENCLOSURE)) {
			content.append(BUTTON_START)
					.append(getString(R.string.see_enclosure))
					.append(BUTTON_MIDDLE)
		...
		if (link != null && link.length() > 0) {
			content.append(LINK_BUTTON_START).append(link)
					.append(LINK_BUTTON_MIDDLE)
					.append(getString(R.string.see_link))
					.append(LINK_BUTTON_END);
		}
		...
		wv.setWebViewClient(new WebViewClient() {
			...
			public void onPageFinished(WebView view, String url) {
				...
				if (mScrollPercentage != 0) {
					view.postDelayed(new Runnable() {
						@Override
						public void run() {
							float webviewsize = wv.getContentHeight()
									- wv.getTop();
							float positionInWV = webviewsize
									* mScrollPercentage;
							int positionY = Math.round(wv.getTop()
									+ positionInWV);
			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				try {
					...
					Toast.makeText(EntryActivity.this, R.string.cant_open_link,
			...
			Cursor cursor = getContentResolver().query(
					...
					PrefUtils.getBoolean(PrefUtils.SHOW_READ, true)
							|| EntryColumns.FAVORITES_CONTENT_URI
									.equals(parentUri) ? null
							: EntryColumns.WHERE_UNREAD, null,
	...
	private void switchEntry(long id, Animation inAnimation,
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (favorite) {
			...
			item.setTitle(R.string.menu_unstar).setIcon(
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_star:
			...
			if (favorite) {
				...
				item.setTitle(R.string.menu_star).setIcon(
		...
		case R.id.menu_copy_clipboard: {
			...
			Toast.makeText(this, R.string.copied_clipboard, Toast.LENGTH_SHORT)
					.show();
			break;
		}
		case R.id.menu_mark_as_unread:
			new Thread() {
				...
				public void run() {
					ContentResolver cr = getContentResolver();
					if (cr.update(uri, FeedData.getUnreadContentValues(), null,
							null) > 0) {
						FeedDataContentProvider.notifyAllFromEntryUri(uri,
								false);
					}
				}
			}.start();
	...
	private class JavaScriptObject {
		...
		public void onClickFullText() {
			if (getProgressBar().getVisibility() != View.VISIBLE) {
				ContentResolver cr = getContentResolver();
				Cursor entryCursor = cr.query(uri, null, null, null, null);
				final boolean alreadyMobilized = entryCursor.moveToFirst()
						&& !entryCursor.isNull(mobilizedHtmlPosition);
				entryCursor.close();
				...
				if (alreadyMobilized) {
					...
					if (networkInfo != null
						...
						long mobilizingTaskId = 1; // FetcherService.getMobilizingTaskId(_id);
						if (mobilizingTaskId != -1) {
							cr.unregisterContentObserver(mTasksObserver);
							cr.registerContentObserver(
									TaskColumns.CONTENT_URI(mobilizingTaskId),
									false, mTasksObserver);
							startService(new Intent(EntryActivity.this,
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					final int position1 = enclosure
							.indexOf(Constants.ENCLOSURE_SEPARATOR);
					final int position2 = enclosure.indexOf(
							Constants.ENCLOSURE_SEPARATOR, position1 + 3);
