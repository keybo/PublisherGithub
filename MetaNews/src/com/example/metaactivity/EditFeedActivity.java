...
import com.example.metaadapter.FiltersCursorAdapter;
...

{
	...
	public void onClickAddFilter(View view) {
		final View dialogView = getLayoutInflater().inflate(
				R.layout.dialog_filter_edit, null);
	...
	public void onClickSearch(View view) {
		final View dialogView = getLayoutInflater().inflate(
				R.layout.dialog_search_feed, null);
						...
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								if (searchText.getText().length() > 0) {
									...
									case R.id.byTopic:
										mUrlEditText
												.setText("http://www.faroo.com/api?q="
														+ text
									...
									case R.id.byYoutube:
										mUrlEditText
												.setText("http://www.youtube.com/rss/user/"
														+ text.replaceAll(
																"\\+", "")
														+ "/videos.rss");
										break;
	...
	public ArrayList<HashMap<String, String>> loadInBackground() {
		try {
			HttpURLConnection conn = NetworkUtils
					.setupConnection("https://ajax.googleapis.com/ajax/services/feed/find?v=1.0&q="
							+ mSearchText);
			try {
					...
					try {
						...
						if (!url.isEmpty()) {
							...
							map.put(EditFeedActivity.FEED_SEARCH_TITLE,
									Html.fromHtml(
											entry.get(
													EditFeedActivity.FEED_SEARCH_TITLE)
													.toString()).toString());
							map.put(EditFeedActivity.FEED_SEARCH_URL, url);
							map.put(EditFeedActivity.FEED_SEARCH_DESC,
									Html.fromHtml(
											entry.get(
													EditFeedActivity.FEED_SEARCH_DESC)
													.toString()).toString());
