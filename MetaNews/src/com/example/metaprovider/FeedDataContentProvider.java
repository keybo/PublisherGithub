...
public class FeedDataContentProvider extends ContentProvider {
	...
	static {
		URI_MATCHER.addURI(FeedData.AUTHORITY, "grouped_feeds",
				URI_GROUPED_FEEDS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "groups", URI_GROUPS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "groups/#", URI_GROUP);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "groups/#/feeds",
				URI_FEEDS_FOR_GROUPS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "feeds", URI_FEEDS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "feeds/#", URI_FEED);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "feeds/#/entries",
				URI_ENTRIES_FOR_FEED);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "feeds/#/entries/#",
				URI_ENTRY_FOR_FEED);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "groups/#/entries",
				URI_ENTRIES_FOR_GROUP);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "groups/#/entries/#",
				URI_ENTRY_FOR_GROUP);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "filters", URI_FILTERS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "feeds/#/filters",
				URI_FILTERS_FOR_FEED);
		...
		URI_MATCHER.addURI(FeedData.AUTHORITY, "favorites/#",
				URI_FAVORITES_ENTRY);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "tasks", URI_TASKS);
		URI_MATCHER.addURI(FeedData.AUTHORITY, "tasks/#", URI_TASK);
	}

	private static final String FEEDS_TABLE_WITH_GROUP_PRIORITY = FeedColumns.TABLE_NAME
			+ " LEFT JOIN (SELECT "
			+ FeedColumns._ID
			+ " AS joined_feed_id, "
			...
			+ ") AS f ON ("
			+ FeedColumns.TABLE_NAME
			+ '.' + FeedColumns.GROUP_ID + " = f.joined_feed_id)";
	private static final String ENTRIES_TABLE_WITH_FEED_INFO = EntryColumns.TABLE_NAME
			+ " JOIN (SELECT "
			+ FeedColumns._ID
			+ " AS joined_feed_id, "
			+ FeedColumns.NAME
			...
			+ '.'
			+ EntryColumns.FEED_ID + " = f.joined_feed_id)";
	...
	public String getType(Uri uri) {
		...
		case URI_FEEDS:
			return "vnd.android.cursor.dir/vnd.metaprovider.feed";
		case URI_GROUP:
		case URI_FEED:
			return "vnd.android.cursor.item/vnd.metaprovider.feed";
		case URI_FILTERS:
		case URI_FILTERS_FOR_FEED:
			return "vnd.android.cursor.dir/vnd.metaprovider.filter";
		case URI_FAVORITES:
		case URI_ENTRIES:
		case URI_ENTRIES_FOR_FEED:
		case URI_ENTRIES_FOR_GROUP:
			return "vnd.android.cursor.dir/vnd.metaprovider.entry";
		case URI_FAVORITES_ENTRY:

		case URI_ENTRY:
		case URI_ENTRY_FOR_FEED:
		case URI_ENTRY_FOR_GROUP:
			return "vnd.android.cursor.item/vnd.metaprovider.entry";
		case URI_TASKS:
			return "vnd.android.cursor.dir/vnd.metaprovider.task";
		case URI_TASK:
			return "vnd.android.cursor.item/vnd.metaprovider.task";
		default:
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		if ((option == URI_FEEDS || option == URI_GROUPS || option == URI_FEEDS_FOR_GROUPS)
				&& sortOrder == null) {
		...
		case URI_GROUPED_FEEDS: {
			...
			sortOrder = "IFNULL(group_priority, " + FeedColumns.PRIORITY
					+ "), IFNULL(" + FeedColumns.GROUP_ID + ", "
					+ FeedColumns._ID + "), " + FeedColumns.IS_GROUP
					+ " DESC, " + FeedColumns.PRIORITY;
		...
		case URI_FEEDS_FOR_GROUPS: {
			queryBuilder.setTables(FeedColumns.TABLE_NAME);
			queryBuilder.appendWhere(new StringBuilder(FeedColumns.GROUP_ID)
		...
		case URI_FILTERS_FOR_FEED: {
			...
			queryBuilder.setTables(FilterColumns.TABLE_NAME);
			queryBuilder.appendWhere(new StringBuilder(FilterColumns.FEED_ID)
					.append('=').append(uri.getPathSegments().get(1)));
			break;
		}
		case URI_ENTRY_FOR_FEED:
		...
		case URI_ENTRY_FOR_GROUP: {
			...
			queryBuilder.appendWhere(new StringBuilder(EntryColumns._ID)
		...
		case URI_ENTRIES_FOR_FEED: {
			...
			queryBuilder.setTables(EntryColumns.TABLE_NAME);
			queryBuilder.appendWhere(new StringBuilder(EntryColumns.FEED_ID)
					.append('=').append(uri.getPathSegments().get(1)));
			break;
		}
		case URI_ENTRIES_FOR_GROUP: {
		...
		case URI_FAVORITES: {
			queryBuilder.setTables(ENTRIES_TABLE_WITH_FEED_INFO);
			queryBuilder
					.appendWhere(new StringBuilder(EntryColumns.IS_FAVORITE)
	...
	private final String[] MAXPRIORITY = new String[] { "MAX("
			+ FeedColumns.PRIORITY + ")" };
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case URI_FEEDS: {
			...
			if (values.containsKey(FeedColumns.GROUP_ID)) {
				...
				cursor = query(
						FeedColumns.FEEDS_FOR_GROUPS_CONTENT_URI(groupId),
						MAXPRIORITY, null, null, null);
			} else {
				cursor = query(FeedColumns.GROUPS_CONTENT_URI, MAXPRIORITY,
						null, null, null);
			}

			if (cursor.moveToFirst()) {
				// normally this is always the case with MAX()
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case URI_FEED: {
			...
			if (values != null && values.containsKey(FeedColumns.PRIORITY)) {
				Cursor priorityCursor = database.query(FeedColumns.TABLE_NAME,
						new String[] { FeedColumns.PRIORITY,
								FeedColumns.GROUP_ID }, FeedColumns._ID + "="
				...
				if (priorityCursor.moveToNext()) {
					...
					String newGroupId = values
							.getAsString(FeedColumns.GROUP_ID);
					...
					String oldGroupWhere = '(' + (oldGroupId != null ? FeedColumns.GROUP_ID
							...
							: FeedColumns.IS_GROUP + Constants.DB_IS_TRUE
									+ Constants.DB_OR + FeedColumns.GROUP_ID
									+ Constants.DB_IS_NULL) + ')';

					// If the group has changed, it is not only a +1 or -1 for
					// priority...
					if ((oldGroupId == null && newGroupId != null)
							|| (oldGroupId != null && newGroupId == null)
							|| (oldGroupId != null && newGroupId != null && !oldGroupId
									.equals(newGroupId))) {

						String priorityValue = FeedColumns.PRIORITY + "-1";
						String priorityWhere = FeedColumns.PRIORITY + '>'
								+ oldPriority;
						database.execSQL("UPDATE " + FeedColumns.TABLE_NAME
								+ " SET " + FeedColumns.PRIORITY + '='
								+ priorityValue + " WHERE " + oldGroupWhere
								+ Constants.DB_AND + priorityWhere);

						priorityValue = FeedColumns.PRIORITY + "+1";
						priorityWhere = FeedColumns.PRIORITY + '>'
								+ (newPriority - 1);
						String newGroupWhere = '(' + (newGroupId != null ? FeedColumns.GROUP_ID
								+ '=' + newGroupId
								: FeedColumns.IS_GROUP + Constants.DB_IS_TRUE
										+ Constants.DB_OR
										+ FeedColumns.GROUP_ID
						...
						database.execSQL("UPDATE " + FeedColumns.TABLE_NAME
								+ " SET " + FeedColumns.PRIORITY + '='
								+ priorityValue + " WHERE " + newGroupWhere
								+ Constants.DB_AND + priorityWhere);
		...
		case URI_FILTERS_FOR_FEED: {
			table = FilterColumns.TABLE_NAME;
			where.append(FilterColumns.FEED_ID).append('=')
					.append(uri.getPathSegments().get(1));
			break;
		}
		case URI_ENTRY_FOR_FEED:
		case URI_ENTRY_FOR_GROUP: {
			...
			where.append(EntryColumns._ID).append('=')
					.append(uri.getPathSegments().get(3));
			break;
		}
		case URI_ENTRIES_FOR_FEED: {
			table = EntryColumns.TABLE_NAME;
			where.append(EntryColumns.FEED_ID).append('=')
					.append(uri.getPathSegments().get(1));
			break;
		}
		case URI_ENTRIES_FOR_GROUP: {
			table = EntryColumns.TABLE_NAME;
			where.append(EntryColumns.FEED_ID).append(" IN (SELECT ")
					.append(FeedColumns._ID).append(" FROM ")
					.append(FeedColumns.TABLE_NAME).append(" WHERE ")
					.append(FeedColumns.GROUP_ID).append('=')
					.append(uri.getPathSegments().get(1)).append(')');
			break;
		}
		case URI_ENTRIES: {
			table = EntryColumns.TABLE_NAME;
		...
		case URI_TASK: {
			table = TaskColumns.TABLE_NAME;
			where.append(TaskColumns._ID).append('=')
					.append(uri.getPathSegments().get(1));
			break;
		}
		}
		...
		int count = database.update(table, values, where.toString(),
				selectionArgs);

		if (FeedColumns.TABLE_NAME.equals(table)
				&& (values.containsKey(FeedColumns.NAME)
						|| values.containsKey(FeedColumns.URL) || values
							.containsKey(FeedColumns.PRIORITY))) {
			// mDatabaseHelper.exportToOPML();
		}
		if (count > 0) {
			getContext().getContentResolver().notifyChange(uri, null);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case URI_GROUP: {
			...
			Cursor subFeedsCursor = database.query(FeedColumns.TABLE_NAME,
					FeedColumns.PROJECTION_ID, FeedColumns.GROUP_ID + "="
			...
			Cursor priorityCursor = database.query(FeedColumns.TABLE_NAME,
					FeedColumns.PROJECTION_PRIORITY, FeedColumns._ID + "="
							+ groupId, null, null, null, null);

			if (priorityCursor.moveToNext()) {
				int priority = priorityCursor.getInt(0);
				String priorityWhere = FeedColumns.PRIORITY + " > " + priority;
				String groupWhere = '(' + FeedColumns.IS_GROUP
						+ Constants.DB_IS_TRUE + Constants.DB_OR
						+ FeedColumns.GROUP_ID + Constants.DB_IS_NULL + ')';
				database.execSQL("UPDATE " + FeedColumns.TABLE_NAME + " SET "
						+ FeedColumns.PRIORITY + " = " + FeedColumns.PRIORITY
						+ "-1 WHERE " + groupWhere + Constants.DB_AND
						+ priorityWhere);
			}
			priorityCursor.close();
			break;
		...
		case URI_FEED: {
			...
			Log.e("Feed id from database" + feedId, "Feed Id from database"
					+ feedId);
			// Remove also the feed entries
			new Thread() {
				@Override
				public void run() {
					Uri entriesUri = EntryColumns
							.ENTRIES_FOR_FEED_CONTENT_URI(feedId);
					delete(entriesUri, null, null);
					delete(FilterColumns.FILTERS_FOR_FEED_CONTENT_URI(feedId),
							null, null);
			...
			// Update the priorities
			Cursor priorityCursor = database
					.query(FeedColumns.TABLE_NAME, new String[] {
							FeedColumns.PRIORITY, FeedColumns.GROUP_ID },
							FeedColumns._ID + '=' + feedId, null, null, null,
							null);
		...
		case URI_FEEDS_FOR_GROUPS: {
			table = FeedColumns.TABLE_NAME;
			where.append(FeedColumns.GROUP_ID).append('=')
					.append(uri.getPathSegments().get(1));
			break;
		}
		case URI_FILTERS: {
	...
	private static String getFeedIdFromEntryId(long entryId) {
		...
		Cursor c = cr.query(EntryColumns.CONTENT_URI(entryId),
				new String[] { EntryColumns.FEED_ID }, null, null, null);
		if (c.moveToFirst()) {
	...
	public static void notifyGroupFromFeedId(String feedId) {
		...
		Cursor c = cr.query(FeedColumns.CONTENT_URI(feedId),
				FeedColumns.PROJECTION_GROUP_ID, null, null, null);
		if (c.moveToFirst()) {
			...
			if (groupId == null) {
				...
				cr.notifyChange(
						FeedColumns.FEEDS_FOR_GROUPS_CONTENT_URI(groupId), null);
			}
		}
		c.close();
	}

	public static void notifyAllFromEntryUri(Uri entryUri,
			boolean onlyStarredOrUnstarred) {
		ContentResolver cr = MainApplication.getContext().getContentResolver();
		...
		try {
			...
			if (!onlyStarredOrUnstarred) {
				String feedId = FeedDataContentProvider
						.getFeedIdFromEntryId(entryId);
				if (feedId != null) {
					FeedDataContentProvider.notifyGroupFromFeedId(feedId);
				}
				if (!uriStr.startsWith(FeedColumns.CONTENT_URI.toString())) {
					cr.notifyChange(ContentUris.withAppendedId(
							FeedColumns.CONTENT_URI, entryId), null);
				}
			}

			if (!uriStr.startsWith(EntryColumns.FAVORITES_CONTENT_URI
					.toString())) {
				cr.notifyChange(ContentUris.withAppendedId(
						EntryColumns.FAVORITES_CONTENT_URI, entryId), null);
			}
			if (!uriStr.startsWith(EntryColumns.CONTENT_URI.toString())) {
				cr.notifyChange(ContentUris.withAppendedId(
						EntryColumns.CONTENT_URI, entryId), null);
			}
